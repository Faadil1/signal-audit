/* ==========================================================================
   SignalAudit — The Gatekeeper Protocol
   Presentation layer only. Scenario facts below are unchanged from the
   prior build and remain copied verbatim from validated backend runs:
...
(function () {
  ...
  var SCENARIOS = {
    abort: {
      ...
      specStatus: "SPEC ISSUED — no entry",
      flags: [
      ...
      statusSub: "Strategy held at checkpoint. Clearance not granted."
    },

    proceed: {
      verdict: "PROCEED",
      bannerText: "CLEARANCE GRANTED",
      regime: "TRENDING_UP (HIGH confidence)",
      specStatus: "SPEC ISSUED — original terms",
      flags: [],
      noTradeConditions: [],
      spec: [
      ...
      statusSub: "Strategy passed inspection. Gate is open."
    }
  };
  ...
  var statusPanelValue = document.getElementById("status-panel-value");
  var statusPanelSub = document.getElementById("status-panel-sub");

  var gateRig = document.getElementById("gate-rig");

  var resultPanel = document.getElementById("result-panel");
  var resultBanner = document.getElementById("result-banner");
  var resultBannerText = document.getElementById("result-banner-text");
  var resultRegime = document.getElementById("result-regime");
  var resultSpecStatus = document.getElementById("result-spec-status");
  ...
  function renderScenario(key) {
    ...
    resultBanner.className = "result-banner " + (key === "abort" ? "banner-abort" : "banner-proceed");
    resultBannerText.textContent = data.bannerText;

    resultRegime.textContent = data.regime;
    resultSpecStatus.textContent = data.specStatus;

    renderFlags(data.flags);
    renderNoTrade(data.noTradeConditions);
    renderSpec(data.spec);

    pillEvidenceMode.textContent = "Evidence sealed: " + data.evidenceMode;
  ...
  // ---------------------------------------------------------------------
  // Sequencing — status panel updates fast (before gate animation settles)
  // so the verdict is legible early for video capture.
  // ---------------------------------------------------------------------
  ...
  function runScenario(key) {
    ...
    // Reset to idle, trigger scan sweep immediately
    gateRig.setAttribute("data-state", "idle");
    statusPanel.setAttribute("data-state", "idle");
    statusPanelValue.textContent = "INSPECTING\u2026";
    statusPanelSub.textContent = "Cross-checking strategy against live signal categories.";
    ...
    // Status panel resolves quickly — this is the fast, video-friendly beat.
    window.setTimeout(function () {
      statusPanel.setAttribute("data-state", key);
      statusPanelValue.textContent = data.statusLabel;
      statusPanelSub.textContent = data.statusSub;
    }, 420);

    // Gate visual completes its motion slightly after, then result card renders.
    window.setTimeout(function () {
